...
const playSong = () => {
    ...
    //injects lyrics to text boxes
    const selector1 = document.querySelector(".ogLang").querySelector(".lyrics");
    selector1.innerText="";
    selector1.innerText=randomSong.eng;
    const selector2 = document.querySelector(".translatedLang").querySelector(".lyrics");
    selector2.innerText="";
    selector2.innerText=randomSong.mand;
    document.querySelector('div.second-container').scrollIntoView({ behavior: "smooth" });
    document.querySelector('.currentlyplaying').innerText = randomSong.name;
}
selectRandomSongs = () => {
    min = Math.ceil(0);
    max = Math.floor(songs.length);
    return songs[Math.floor(Math.random() * (max - min)) + min];
};
